...
# cd /media/nas/temporary/PUB_CRCs/tallCRCs/mouse/fastq
# ls *.gz | parallel --progress --eta -j 32 "rsync --ignore-existing -arzRP {} ${fastqDir}"
# cd -

# 1.2) Run the pipeline
cd ${projDir}
nextflow run /home/rad/users/gaurav/projects/workflows/nfatacseq --input ${projDir}/${projName}_design.csv --genome GRCm38 --single_end --narrow_peak -resume -name ${projName}
...
do 
  echo ${b}
  macs2 callpeak -f BAM --seed=39751 -g ${genome} --keep-dup auto -p ${pvalue} --cutoff-analysis -t ${b} -n $(basename ${b} _R1.mLb.clN.sorted.bam)_${pd} --outdir ${peaksOutDir}
done
...
# +-----------------------------------------+--------+--------+-------------------+
# | H3K27ac_B_0_005_cutoff_analysis.txt     |      5 |   9276 |           0.00001 |
# | H3K27ac_CD4_0_005_cutoff_analysis.txt   |      3 |   5578 |             0.001 |
# | H3K27ac_CD8_0_005_cutoff_analysis.txt   |      3 |   3397 |             0.001 |
# | H3K27ac_CLP_0_005_cutoff_analysis.txt   |     10 |  10000 |      0.0000000001 |
# | H3K27ac_CMP_0_005_cutoff_analysis.txt   |     10 |   4000 |      0.0000000001 |
# | H3K27ac_DP1_0_005_cutoff_analysis.txt   |     15 |        | 0.000000000000001 |
# | H3K27ac_DP2_0_005_cutoff_analysis.txt   |     15 |        | 0.000000000000001 | 
# | H3K27ac_EL4_0_005_cutoff_analysis.txt   |     15 |        | 0.000000000000001 |
# | H3K27ac_GN_0_005_cutoff_analysis.txt    |     10 |   3250 |      0.0000000001 |
# | H3K27ac_MF_0_005_cutoff_analysis.txt    |     10 |   6570 |      0.0000000001 |
# | H3K27ac_STHSC_0_005_cutoff_analysis.txt |      5 |   9515 |           0.00001 |
# +-----------------------------------------+--------+--------+-------------------+

pd=0_001; pvalue="${pd//_/.}"; peaksOutDir=${customPeaksDir}/p${pd}; mkdir -p ${peaksOutDir}
for s in  H3K27ac_CD4_ H3K27ac_CD8_; do bam=$(find ${bamDir} -iname *${s}*.bam); macs2 callpeak -f BAM --seed=39751 -g ${genome} --keep-dup auto -p ${pvalue} --cutoff-analysis -t ${bam} -n $(basename ${bam} _R1.mLb.clN.sorted.bam)_p${pd} --outdir ${peaksOutDir}; done

pd=0_00001; pvalue="${pd//_/.}"; peaksOutDir=${customPeaksDir}/p${pd}; mkdir -p ${peaksOutDir}
for s in  H3K27ac_B_ H3K27ac_STHSC_; do bam=$(find ${bamDir} -iname *${s}*.bam); macs2 callpeak -f BAM --seed=39751 -g ${genome} --keep-dup auto -p ${pvalue} --cutoff-analysis -t ${bam} -n $(basename ${bam} _R1.mLb.clN.sorted.bam)_p${pd} --outdir ${peaksOutDir}; done

pd=0_0000000001; pvalue="${pd//_/.}"; peaksOutDir=${customPeaksDir}/p${pd}; mkdir -p ${peaksOutDir}
for s in  H3K27ac_CLP_ H3K27ac_CMP_ H3K27ac_GN_ H3K27ac_MF_ ; do bam=$(find ${bamDir} -iname *${s}*.bam); macs2 callpeak -f BAM --seed=39751 -g ${genome} --keep-dup auto -p ${pvalue} --cutoff-analysis -t ${bam} -n $(basename ${bam} _R1.mLb.clN.sorted.bam)_p${pd} --outdir ${peaksOutDir}; done

pd=0_000000000000001; pvalue="${pd//_/.}"; peaksOutDir=${customPeaksDir}/p${pd}; mkdir -p ${peaksOutDir}
for s in  H3K27ac_DP1_ H3K27ac_DP2_ H3K27ac_EL4_ ; do bam=$(find ${bamDir} -iname *${s}*.bam); macs2 callpeak -f BAM --seed=39751 -g ${genome} --keep-dup auto -p ${pvalue} --cutoff-analysis -t ${bam} -n $(basename ${bam} _R1.mLb.clN.sorted.bam)_p${pd} --outdir ${peaksOutDir}; done
...
for pd in 0_001 0_00001 0_0000000001 0_000000000000001;
do
